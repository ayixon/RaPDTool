...
import ruamel_yaml
from ruamel_yaml.error import UnsafeLoaderWarning, YAMLError  # NOQA
...
from ruamel_yaml.compat import StringIO, BytesIO, with_metaclass, PY3, nprint
from ruamel_yaml.resolver import VersionedResolver, Resolver  # NOQA
from ruamel_yaml.representer import (
    BaseRepresenter,
    SafeRepresenter,
    Representer,
    RoundTripRepresenter,
)
from ruamel_yaml.constructor import (
    BaseConstructor,
    SafeConstructor,
    Constructor,
...
class YAML(object):
    def __init__(
        ...
        if self.typ == 'rt':
            ...
            self.Emitter = (
                ruamel_yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
    ...
    def get_serializer_representer_emitter(self, stream, tlca):
        ...
        rslvr = (
            ruamel_yaml.resolver.BaseResolver
            if self.typ == 'base'
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, 'add_implicit_resolver'):
            ...
            Loader, (BaseLoader, SafeLoader, ruamel_yaml.loader.Loader, RoundTripLoader)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, 'add_implicit_resolver'):
            ...
            Dumper, (BaseDumper, SafeDumper, ruamel_yaml.dumper.Dumper, RoundTripDumper)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
