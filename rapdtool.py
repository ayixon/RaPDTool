...
def setAbsPaths(rootPath):
    global scriptHomePath, inputPath, profilesPath, genomadbPath, workLogFocusPath, \
        ...
        workLogBinningrefPath, workOutMashPath, resultPath, processedPath
    ...
    resultPath = rootPath + 'resultfmbm/'
    ...
    try:
        for aPath in (scriptHomePath, inputPath, profilesPath, genomadbPath, workPath, \
            ...
            workOutBinningrefPath, workLogBinningrefPath, workOutMashPath, resultPath, \
            processedPath):
...
def setFileDepPaths(filename, bluntname):
    global focusOutPath, focusLogFile, metabatBinPath, metabatLogFile, binningrefInPath, \
        ...
        binningrefResultRefBinsPath, binningrefLogFile, mashOutPath, resultFile
    ...
    resultFile = resultPath + filename + '.out'

    try:
        for path in (focusOutPath, metabatBinPath, binningrefInPath,
            binningrefInOnePath, binningrefInTwoPath, mashOutPath ):
...
def extractMinDistRows(mashTaxoMatchList):
    want = 10
    minDistRows = []
    for mtm in mashTaxoMatchList:
        ...
        for line in open(mashOutPath + mtm, 'r'):
            ...
            linitemslist.append(mtm) # [5] 6th item -> the file name
            linitemslist.append( str(rown) ) # [6] 7th item -> the row number
            linitemslist.append(distn) # [7] 8th item, numeric distance
            insidx = len(minDistRows) # comparison starting position (next position beyond end of minDistRows)
            while insidx - 1 >= 0 and distn < minDistRows[insidx - 1][7]:
            ...
            if len(minDistRows) > want: # if insertion made top set overflow, discard the trailing row
                minDistRows.pop()
    message = 'All ' + str(len(mashTaxoMatchList)) + ' files were examined for the best ' + str(want) + ' distances'
    return (minDistRows, message) 

def writeBestDistReport(minDistRows, printableComment):
    fobj = open(resultFile, 'w')
    for row in minDistRows:
        fobj.write('\t'.join(row[:7]) + '\n')
...
appendLog('MinDistance extraction - start')
(minDistRows, message) = extractMinDistRows(mashTaxoMatchList)
appendLog(message)

appendLog('Writing best distances report')
writeBestDistReport(minDistRows, printableComment)
